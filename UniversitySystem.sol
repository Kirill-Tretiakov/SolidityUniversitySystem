...
abstract contract AttendanceAndGrades is UserBase, CourseBase {
    ...
    function getGradesStudent(uint _courseId) public view onlyStudent returns (uint[] memory) {
        require(users[msg.sender].role == Role.Student, "Only students can access their grades");

        uint[] memory grades = new uint[](courses[_courseId].sessions.length);
        
        for (uint i = 0; i < courses[_courseId].sessions.length; i++) {
            grades[i] = courses[_courseId].sessions[i].grades[msg.sender];
        }
        return grades;
    }
...
abstract contract ReportsManager is UserBase, CourseBase {
    ...
    function viewCourseStatistics(uint _courseId) external view returns (uint averageGrade, uint attendanceRate) {
        ...
        uint avgGrade = (course.totalGrades * 100) / (course.sessionCount * course.participantCount);
